...
class Board:
    ...
    def __init__(self, matrix, rows, cols, squares_left_row, squares_left_col, boats):
        ...
        self.rows = rows
        self.cols = cols
        self.squares_left_row = squares_left_row
        self.squares_left_col = squares_left_col
        self.boats = boats
        self.invalid = False
    ...
    def insert_water(self, row: int, col: int):
        if 0 <= row < 10 and 0 <= col < 10 and self.get_value(row, col) == None:
            ...
            if h[0] in ('L', 'l') or h[1] in ('R', 'r') or v[0] in ('T', 't') or v[1] in ('B', 'b'):
                self.invalid = True
    ...
    def check_completed(self) -> bool:
        if self.invalid or not self.full_boats:
            return False
        for i in range(0, 10):
    ...
    def complete_if_possible(self, row: int, col: int):
        value = self.get_value(row, col)
        ...
        if value in ('T', 't'):
            ...
            if aux1 == None:
                if row == 8 or aux2 in ('.', 'W') or self.diff_cols(col) == 1:
            ...
            aux2 = self.get_value(row-2,col)
            if aux1 == None:
                if row == 1 or aux2 in ('.', 'W') or self.diff_cols(col) == 1:
            ...
            if aux1 == None:
                if col == 8 or aux2 in ('.', 'W') or self.diff_rows(row) == 1:
            ...
            aux2 = self.get_value(row,col-2)
            if aux1 == None:
                if col == 1 or aux2 in ('.', 'W') or self.diff_rows(row) == 1:
            ...
            if '.' in self.adjacent_horizontal_values(row,col) or 'W' in self.adjacent_horizontal_values(row,col):
                ...
                if aux_1 == None and aux1 == None and (self.diff_cols(col) == 2 or (aux_2 in ('.', 'W') and aux2 in ('.', 'W'))):
                    ...
                    self.insert_boat(row+1, col, 'b')
                    self.boats[2] -= 1
                if aux1 == None and (row == 8 or aux2 in ('.', 'W') or self.diff_cols(col) == 1):
                    self.insert_boat(row+1, col, 'b')
                if aux_1 == None and (row == 1 or aux_2 in ('.', 'W') 
                                      or self.diff_cols(col) == 1):
                    self.insert_boat(row-1, col, 't')
                ...
                if aux_1 == None and aux1 == None and (self.diff_cols(col) == 2 or (aux_2 in ('.', 'W') and aux2 in ('.', 'W'))):
                    ...
                    self.boats[2] -= 1
                elif aux1 == None and (col == 8 or aux2 in ('.', 'W') or self.diff_rows(row) == 1):
                    self.insert_boat(row, col+1, 'r')
                elif aux_1 == None and (col == 1 or aux_2 in ('.', 'W') or self.diff_rows(row) == 1):
                    self.insert_boat(row, col-1, 'l')

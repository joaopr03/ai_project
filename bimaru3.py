...
def is_boat(value) -> bool:
    return value != '.' and value != 'W' and value != None


class Board:
    """Representação interna de um tabuleiro de Bimaru."""

    def __init__(self, matrix, rows, cols, squares_left_row, squares_left_col, barco_1):
        self.matrix = matrix
    ...
    def value(self, row: int, col: int) -> str:
        ...
        if is_boat(h[0]): # ao lado de um left ou middle
            if col == 9 or h[1] in ('.', 'W') or self.rows[row][0] - self.rows[row][1] == 1:
                return 'r'
        if is_boat(h[1]): # ao lado de um right ou middle
            if col == 0 or h[0] in ('.', 'W') or self.rows[row][0] - self.rows[row][1] == 1:
                return 'l'
        
        if is_boat(v[0]) and is_boat(v[1]):
            return 'm'
        if is_boat(v[0]): # ao lado de um top ou middle
            if row == 9 or v[1] in ('.', 'W') or self.cols[col][0] - self.cols[col][1] == 1:
                return 'b'
        if is_boat(v[1]): # ao lado de um bottom ou middle
            if row == 0 or v[0] in ('.', 'W') or self.cols[col][0] - self.cols[col][1] == 1:
                return 't'
    ...
    def insert_boat(self, action):
        ...
        if 0 <= row < 10 and 0 <= col < 10 and self.get_value(row, col) == None:
            ...
            self = self.verify_completed_boat(row, col, value)
            self.squares_left_row[row] -= 1
            self.squares_left_col[col] -= 1
            self.rows[row][1] += 1
            self.cols[col][1] += 1
        ...
        for i in range(0,10):
            if self.get_value(row, i) == None:
                ...
                if self.full_col(i) and self.squares_left_col[i] != 0:
                    self.complete_col_with_water(i)
                    for j in range(0,10):
                        if self.get_value(j, i) == 'x':
                            value = self.new_value(j, i)
                            self.matrix[j][i] = value
                            self = self.verify_completed_boat(j, i, value)
        for i in range(0,10):
            if self.get_value(row, i) == 'x':
                value = self.new_value(row, i)
                self.matrix[row][i] = value
                self = self.verify_completed_boat(row, i, value)
                if value in ('t', 'm', 'b'):
                    for j in range(row-3, row+3):
                        if self.get_value(j, i) == 'x':
                            self.matrix[j][i] = self.new_value(j, i)
                            self = self.verify_completed_boat(j, i, self.get_value(j,i))
        return self
                ...
                if self.full_row(i) and self.squares_left_row[i] != 0:
                    ...
                    for j in range(0,10):
                        if self.get_value(i, j) == 'x':
                            value = self.new_value(i, j)
                            self.matrix[i][j] = value
                            self = self.verify_completed_boat(i, j, value)
        for i in range(0,10):
            if self.get_value(i, col) == 'x':
                value = self.new_value(i, col)
                self.matrix[i][col] = value
                self = self.verify_completed_boat(i, col, value)
                if value in ('l', 'm', 'r'):
                    for j in range(col-3, col+3):
                        if self.get_value(i, j) == 'x':
                            self.matrix[i][j] = self.new_value(i, j)
                            self = self.verify_completed_boat(i, j, self.get_value(i,j))          
        return self

    def verify_completed_boat(self, row:int, col:int, value:str):
        if value == "c":
            self.barco_1 -= 1
        elif value == "t":
            if self.get_value(row + 1, col) in ("b", "B"):
                self.barco_2 -= 1
            elif self.get_value(row + 1, col) in ('m', 'M', 'x') and self.get_value(row + 2, col) in ("b", "B"):
                self.barco_3 -= 1
            elif self.get_value(row + 1, col) in ('m', 'M', 'x') and self.get_value(row + 2, col) in ('m', 'M', 'x') and self.get_value(row + 3, col) in ("b", "B"):
                self.barco_4 -= 1
        elif value == "b":
            if self.get_value(row - 1, col) in ('t', 'T'):
                self.barco_2 -= 1
            elif self.get_value(row - 1, col) in ('m', 'M', 'x') and self.get_value(row - 2, col) in ('t', 'T'):
                self.barco_3 -= 1
            elif self.get_value(row - 1, col) in ('m', 'M', 'x') and self.get_value(row - 2, col) in ('m', 'M', 'x') and self.get_value(row - 3, col) in ('t', 'T'):
                self.barco_4 -= 1
        elif value == "l":
            if self.get_value(row, col + 1) in ("r", "R"):
                self.barco_2 -= 1
            elif self.get_value(row, col + 1) in ('m', 'M', 'x') and self.get_value(row, col + 2) in ("r", "R"):
                self.barco_3 -= 1
            elif self.get_value(row, col + 1) in ('m', 'M', 'x') and self.get_value(row, col + 2) in ('m', 'M', 'x') and self.get_value(row, col + 3) in ("r", "R"):
                self.barco_4 -= 1
        elif value == "r":
            if self.get_value(row, col - 1) in ("l", "L"):
                self.barco_2 -= 1
            elif self.get_value(row, col - 1) in ('m', 'M', 'x') and self.get_value(row, col - 2) in ("l", "L"):
                self.barco_3 -= 1
            elif self.get_value(row, col - 1) in ('m', 'M', 'x') and self.get_value(row, col - 2) in ('m', 'M', 'x') and self.get_value(row, col - 3) in ("l", "L"):
                self.barco_4 -= 1
        if self.barco_1 < 0 or self.barco_2 < 0 or self.barco_3 < 0 or self.barco_4 < 0:
            self.invalid = True
        return self

    def possible_actions(self):
        for i in range(0, 10):
            diff_r = self.rows[i][0] - self.rows[i][1]
            if diff_r != 0 and self.squares_left_row[i] != 0 and diff_r == self.squares_left_row[i]:
                return [('complete_boats_row', i)]
            diff_v = self.cols[i][0] - self.cols[i][1]
            if diff_v != 0 and self.squares_left_col[i] != 0 and diff_v == self.squares_left_col[i]:
    ...
    def parse_instance():
        ...
        return Board(matrix, new_rows, new_cols, slrow, slcol, barco_1).start_board()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        '''print(b1.board.barco_1, b1.board.barco_2, b1.board.barco_3, b1.board.barco_4)
        print(b1.board)'''
